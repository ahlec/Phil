...
export default class InfoConfigAction<TModel> implements IConfigAction<TModel> {
    ...
    public readonly description = `see detailed information about a configuration property ${
    ...
    public async process(command: ConfigCommandBase<TModel>, phil: Phil, message: PublicMessage,
        mutableArgs: string[], property: IConfigProperty<TModel>, model: TModel): Promise<any> {
            ...
            response += `${
                ...
                ConfigActionPrimaryKey.Set} ${property.key} ${randomDisplayValue}${
                NEWLINE}${message.serverConfig.commandPrefix}${command.name} ${
