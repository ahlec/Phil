...
type GetUserResult = { success: true; user: DiscordIOUser; } | { success: false; error: string; };
    ...
    public async processMessage(phil: Phil, message: PublicMessage, commandArgs: ReadonlyArray<string>): Promise<any> {
        if (!message.serverConfig.welcomeMessage) {
            return BotUtils.sendErrorMessage({
                ...
                message: `Your server is not configured with a welcome message. An admin can ${
                    NOWRAP}change this by using \`${message.serverConfig.commandPrefix}config set ${
                    NOWRAP}welcome-message\`.`
            });
        }
        ...
        return DiscordPromises.sendMessage(phil.bot, message.channelId, welcomeMessage);
    }

    private getUser(phil: Phil, message: PublicMessage, commandArgs: ReadonlyArray<string>): GetUserResult {
        ...
        const validityResult = MemberTypeDefinition.isValid(userId, phil, message.serverConfig);
