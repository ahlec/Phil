...
export default class UnconfirmedPromptTally {
    public static async collectForServer(db: Database, serverId: string) : Promise<UnconfirmedPromptTally[]> {
        const results = await db.query(`SELECT pb.bucket_id, reference_handle, display_name, count(*) as "total"
                FROM prompts p
        ...
        return results.rows.map(row => new UnconfirmedPromptTally(row));
    }
