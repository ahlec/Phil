...
export default class ConfirmationStage implements IStage {
    ...
    public async processInput(phil: Phil, message: PrivateMessage): Promise<any> {
        ...
        if (content === 'yes') {
            return QuestionnaireStageUtils.setStage(phil, message.userId, Stages.Finished);
        }
    ...
    private async getConfirmationMessage(db: Database, userId: string, messagePrefix: string): Promise<string> {
        ...
        let message = messagePrefix + ' If I understand you correctly, your current time should be **';
