...
export default class Feature {
  ...
  public async setIsEnabled(
    ...
    const results = await db.query(
      `UPDATE server_features
                SET is_enabled = $1
                WHERE server_id = $2 AND feature_id = $3`,
      [enabledBit, serverId, this.id]
    );
    if (results.rowCount > 0) {
      return;
    }
  ...
  private async insertNewDatabaseRow(
    db: Database,
    serverId: string,
    enabledBit: number
  ): Promise<void> {
    const results = await db.query(
      `INSERT INTO
                ...
                VALUES($1, $2, $3)`,
      [serverId, this.id, enabledBit]
    );
    if (results.rowCount !== 1) {
      throw new Error(
        'Unable to insert a new record into the `server_features` database.'
      );
    }
  }
}
