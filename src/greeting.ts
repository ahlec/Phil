...
function getUser(bot: DiscordIOClient, member: DiscordIOMember): DiscordIOUser {
  ...
  if (!user) {
    throw new Error(
      'Unable to retrieve the user for the member who just joined.'
    );
  }
...
async function onError(
  ...
  await DiscordPromises.sendMessage(
    ...
    'There was an error sending a greeting message for new member ' +
      member.id +
      '.'
  );
  await DiscordPromises.sendMessage(
...
async function shouldWelcomeMember(
  db: Database,
  serverConfig: ServerConfig,
  user: DiscordIOUser
): Promise<boolean> {
  ...
  if (
    ...
    serverConfig.welcomeMessage.length === 0
  ) {
    return false;
  }
...
export async function greetNewMember(
  phil: Phil,
  serverConfig: ServerConfig,
  member: DiscordIOMember
) {
  try {
    ...
    const shouldWelcome = await shouldWelcomeMember(
      phil.db,
      serverConfig,
      user
    );
